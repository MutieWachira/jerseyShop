...
export default function NewCategory() {
  ...
  const createCategory = async () => {
    ...
    if (!res.ok) {
      ...
      setError(errorBody?.error || "Unable to create category. Please try again.");
      setLoading(false);
      return;
    }
      ...
      <div className="text-slate-900 p-6 lg:ml-64 bg-slate-50 min-h-screen">
        <div className="mx-auto max-w-3xl rounded-[2.5rem] border border-rose-200 bg-rose-50 p-10 text-center shadow-sm">
          ...
          <p className="mt-4 text-sm leading-6 text-rose-800">
            You need to sign in with an administrator account before creating categories.
          </p>
          <button
            onClick={() => router.push("/login?callbackUrl=/admin/categories")}
      ...
      <div className="mx-auto max-w-3xl space-y-8">
        <div className="rounded-[2.5rem] border border-slate-200 bg-white p-10 shadow-2xl shadow-slate-200/50">
          <div className="space-y-4">
            <p className="text-xs uppercase tracking-[0.35em] text-slate-400">Category builder</p>
            <h1 className="text-4xl font-black text-slate-950">Create a new category</h1>
            <p className="max-w-2xl text-sm leading-6 text-slate-500">
              Add a new category to organize products by type, team, or collection.
            </p>
          </div>
          ...
          <div className="mt-10 rounded-[2rem] border border-slate-200 bg-slate-50 p-6">
            ...
            <div className="mt-4 rounded-3xl bg-white px-4 py-3 text-sm text-slate-600 ring-1 ring-slate-200">
              ...
              {slugPreview || "Category slug will appear here"}
            </div>
